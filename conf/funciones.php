...
function idiomaPagina()
{
	...
	$_SESSION["ruta"] = $ruta.="/";
	//Servidor local
	$_SESSION["rutaservidor"] = "http://".$_SERVER["SERVER_NAME"]."/nosvamosdeviajema/";
	//Servidor internet
...
function obtenerMunicipio($link, $idMunicipio, $soloMunicipio){
	$query="select m.Municipio, p.Provincia ";
	$query.="from municipios m inner join provincias p on m.IdProvincia=p.IdProvincia ";
	$query.="where m.IdMunicipio = ".$idMunicipio;
	
	$municipioVisitado=mysqli_query ($link, $query);
	$rowmunicipio=mysqli_fetch_array($municipioVisitado);
	
	$municipio = $rowmunicipio["Municipio"];
	$provincia = $rowmunicipio["Provincia"];

	mysqli_free_result($municipioVisitado);
